...
fuel :: Int -> Info -> Info
fuel qtdGas (Info (Car pass maxPass gas maxGas km) _) = if gas < maxGas then
                                    if gas + qtdGas < maxGas then Info (Car pass maxPass (gas + qtdGas) maxGas km) (Op "fuel" True)
                                    else Info (Car pass maxPass maxGas maxGas km) (Op "fuel" True)
                                else Info (Car pass maxPass gas maxGas km) (Op "fuel" False)

-- Faz entrar uma pessoa no carro. Retorna false se já estiver lotado.
embark :: Info -> Info
embark (Info (Car pass maxPass gas maxGas km) _)
    | pass < maxPass = Info (Car (pass + 1) maxPass gas maxGas km) (Op "embark" True)
    | otherwise = Info (Car pass maxPass gas maxGas km) (Op "embark" False)

-- Retira uma pessoa do carro, retorna false se não tiver ninguém no carro
disembark :: Info -> Info
disembark (Info (Car pass maxPass gas maxGas km) _)
    | pass > 0 = Info (Car (pass - 1) maxPass gas maxGas km) (Op "disembark" True)
    | otherwise = Info (Car pass maxPass gas maxGas km) (Op "disembark" False)
...
drive :: Int -> Info -> Info
drive distance (Info (Car pass maxPass gas maxGas km) _)
    | pass > 0 && gas >= distance = Info (Car pass maxPass (gas - distance) maxGas (km + distance)) (Op "drive" True)
    | pass > 0 && gas > 0 = Info (Car pass maxPass 0 maxGas (km + gas)) (Op "drive" False)
